...
def inventory(catalog,subdir,symbols,as_of,max10k,max10q):
 ...
 return out
def validate_candidate(manifest,root):
 if not isinstance(manifest,dict) or not manifest.get('candidates'): raise ValueError('invalid candidate manifest')
 for company in manifest['candidates']:
  for row in company.get('selected',[]):
   if row.get('form') not in FORMS or row.get('ticker')!=company.get('ticker') or row.get('cik')!=str(company.get('cik')): raise ValueError('candidate identity conflict')
...
def download_corpus(manifest,outdir,user_agent):
 if not user_agent or '@' not in user_agent: raise ValueError('FINSIGHT_SEC_USER_AGENT is required')
 root=root_path(); out=safe_out(outdir,root); out.mkdir(parents=True,exist_ok=True); client=SECClient(SECConfig(user_agent=user_agent)); total=0; stats={'request_count':0,'retry_count':0,'redirect_count':0,'rate_limit_count':0,'timeout_count':0,'downloaded_bytes':0,'failed_documents':[]}; company=[]
 for co in manifest['candidates']:
  ...
  for row in selected:
   try:
    ...
    total+=len(raw); stem=row['accession_number'].replace('-',''); rawp=rawd/(stem+'.bin'); rawp.write_bytes(raw); meta=dict(row,retrieved_at=__import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat(),raw_sha256=digest,input_file=str(rawp.relative_to(root)))
...
def main():
 ...
 if a.network:
  ...
  cm=safe_file(a.candidate_manifest,root); manifest=json.loads(cm.read_text()); validate_candidate(manifest,root); print(json.dumps(download_corpus(manifest,a.output_dir,os.getenv('FINSIGHT_SEC_USER_AGENT')),ensure_ascii=False)); return
