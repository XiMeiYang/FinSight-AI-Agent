...
def saved(a):
 ...
 if market.get('symbol','').strip().upper()!='NVDA' or str(facts.get('cik')).zfill(10)!='0001045810' or str(filings.get('cik')).zfill(10)!='0001045810': raise ValueError('saved input identity mismatch')
 market=dict(market,raw_sha256=mr['sha256'],normalized_sha256=mn['sha256'],input_file=mn['input_file']); facts=dict(facts,normalized_sha256=fn['sha256'],input_file=fn['input_file']); filings=dict(filings,normalized_sha256=sn['sha256'],raw_sha256=sr['sha256'],input_file=sn['input_file'],source_url='https://data.sec.gov/submissions/CIK0001045810.json')
 snap=build_research_snapshot(symbol='NVDA',market_data=market,company_facts=facts,filings=filings,as_of=a.as_of,data_mode='saved_snapshot')
 snap['sources']=[
 ...
 snap['run_record'].update(selected_files=[mn['input_file'],mr['input_file'],fn['input_file'],fr['input_file'],sn['input_file'],sr['input_file']],file_hash_verification='passed',identity_verification='passed',point_in_time_filter={'as_of':snap['as_of'],'filtered_count':snap['data_quality']['point_in_time_filtered_count']},snapshot_write='pending')
 return snap
def main():
 ...
 for n in ('market-normalized','market-raw','sec-companyfacts-normalized','sec-companyfacts-raw','sec-submissions-normalized','sec-submissions-raw'): p.add_argument('--'+n,dest=n.replace('-','_'))
 a=p.parse_args()
